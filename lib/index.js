...
try {
  ...
  ghContext = require('./github-context.sample');
}
...
const gitCommit = ghContext.event.head_commit;
...
server.get('/api/messages/:name?', (req, res, next) => {
  ...
  res.send({ siteName, azureResGroup, pkgVersion, OS, isWindows, isAzure, httpHost, scmType, hello: req.params, deployEvent, gitCommit });
